// static/js/module/service_v2.js
...
const servicesModule = {
    ...
    // NEW: Fetch and cache automotive parts
    ...
    async showBasicQuoteForm() {
        ...
        await this.fetchAutomotiveParts();

        const workDescription = prompt('Enter a brief description for the work:');
        if (!workDescription) return; // Abort if user cancels
        ...
        const quoteModalContent = `
            <form id="newQuoteForm" class="form-grid">
                ...
                <div style="grid-column: 1 / -1; text-align: left;">
                    <button type="button" class="button button-secondary" onclick="servicesModule.addQuoteItemRow()">
                        ➕ Add Item
                    </button>
                </div>

                <div class="quote-totals" style="grid-column: 1 / -1; text-align: right; margin-top: 1rem;">
                    <h4>Subtotal: <span id="quoteSubtotal">฿0.00</span></h4>
                    <h3>Total: <span id="quoteTotal">฿0.00</span></h4>
                </div>
        ...
        // Add the first row automatically and set up event listeners
        setTimeout(() => {
            ...
            table.addEventListener('change', () => this.updateQuoteTotals());
            table.addEventListener('input', () => this.updateQuoteTotals());
    ...
    addQuoteItemRow() {
        ...
        const partsOptions = this.automotiveParts.map(part => 
            `<option value="${part.code}" data-price="${part.price}">
                ${part.thai} / ${part.english} (${part.code}) - ฿${part.price}
            </option>`
        ).join('');
        ...
        row.innerHTML = `
            ...
            <td>
                <input type="number" class="form-input quote-item-qty" value="1" min="1" style="width: 80px;">
            </td>
            <td class="quote-item-price">฿0.00</td>
            <td class="quote-item-total">฿0.00</td>
            <td>
                <button type="button" class="btn-icon danger" onclick="this.closest('tr').remove(); servicesModule.updateQuoteTotals();">
                    🗑️
                </button>
            </td>
        `;
    ...
    updateQuoteTotals() {
        let subtotal = 0;
        const rows = document.querySelectorAll('#quoteItemsTable tr');
        ...
        document.getElementById('quoteTotal').textContent = `฿${subtotal.toFixed(2)}`; // Assuming no tax for now
    },

    handleQuoteSubmission(formData) {
        const quoteNumber = formData.get('quote_number');
        this.showToast(`Quote ${quoteNumber} created successfully!`, 'success');
        if (typeof window.closeModal === 'function') {
            window.closeModal();
        }
        // In a real app, you would now send this data to the server.
        console.log('Quote data to be submitted:', Object.fromEntries(formData.entries()));
    },
